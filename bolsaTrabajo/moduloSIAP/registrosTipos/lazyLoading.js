...
let dataNacion2 = saveDataState() // SE GENERA COMPONENTE GLOBAL PARA NACIONES (Catalogos)
...
setStorageData().then(r => console.log("Catalogos iniciados")) //catlgos para paises o estados, se carga al inicio para evitar errores
...
datosRequest(`obtenerinfoUsuario/${idUser}`, 'GET', 'buttons-data-gral', 'section-skeleton_datos-grales', statusDatosGenerales).then(r => {
    //showDocumentacionPersonal() // Agregando documentacion documentacion...
...
}); /// -----> se comento la linea anterior para evitar primera seccion

/*
async function datosGenerales () {
    let documentacion = await showDocumentacionPersonal() // Agregando documentacion documentacion...
...
//Esta funcion se comento... al inicio.....
async function setStorageData2(){
    dataNacion2.clearDataCountry();
    dataNacion2.clearDataState();
    let getPaises = await getDataFromServerHidden('obtenerNacimientoPais','POST')
    if (getPaises.length > 200){

        let getEstados = await getDataFromServerHidden('obtenerNacimientoEntidadF','POST')
        //Guardando datos en state...
        dataNacion2.setDataCountry(getPaises);
        dataNacion2.setDataState(getEstados);
    }
    return true;
}
...
async function getDatosNacimiento(){
    ...
    let getEstadoNacimiento = await getDataFromServerHidden(`obtenerNacimientoUsuario/${idUser}`,'GET')
...
async function getDatosSeguridadSocial(){
    ...
    let info = await getDataFromServerHidden(`obtenerSegSocialPersona/${idUser}`, 'GET');
