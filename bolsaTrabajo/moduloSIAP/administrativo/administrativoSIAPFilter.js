...
async function sendDataInvitation(element) {
    let id = await $.ajax({
        type: "POST",
        url: "/reclutamientoAdministrativo/enviarInvitacionAdmin",
        data: {
...
async function returnID2(element, element2) {
    let id = await $.ajax({
        type: "POST",
        url: "/reclutamientoAdministrativo/idVerificationAdmin",
        data: {
            _token: $('meta[name="csrf-token"]').attr("content"),
            name: element.name,
            firstName: element.firstName,
...
function getItemsSearch(nombres, apPaterno, apMaterno) {
...
function formSeach (){
    ...
    //Using destructuring
    let [key, nombre, apPaterno, apMaterno] = arrayCollecion;

    let objData = new getItemsSearch(nombre.value, apPaterno.value,apMaterno.value);
    return objData;

}
//Page search
async function busquedaPerfilesAD(){
    ...
    let id = await $.ajax({
        type: 'POST',
        url:'/reclutamientoAdministrativo/busquedaPerfiAd',
        data:{
            _token: $('meta[name="csrf-token"]').attr("content"),
            ...
            data.forEach((item)=>{
                values +=
                    `<tr>
                        <td class="sorting_1"><div class="btn-group mr-2"><a title="Ver detalles" class="btn btn-secondary boton" href="/reclutamientoAdministrativo/detalleProspecto/${item.idPersona}"><i class="fas fa-id-card"></i>   </a></div></td>
