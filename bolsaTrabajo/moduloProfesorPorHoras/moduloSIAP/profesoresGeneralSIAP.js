...
async function sendDataInvitation(element) {
    let id = await $.ajax({
        type: "POST",
        url: "/envioInvitacionCC/enviarInvitacionProfesor",
        data: {
...
async function returnID2(element, element2) {
    let id = await $.ajax({
        type: "POST",
        url: "/envioInvitacionCC/idVerificationProfesor",
        data: {
            _token: $('meta[name="csrf-token"]').attr("content"),
            name: element.name,
            firstName: element.firstName,
...
//Busqueda
function getItemsSearch(nombres, apPaterno, apMaterno, curp,fechaInicio, fechaTermino) {
...
function formSeach (){
    ...
    //Using destructuring
    let [key, nombre, apPaterno, apMaterno, curp, fechaInicio, fechaTermino] = arrayCollecion;

    let objData = new getItemsSearch(nombre.value, apPaterno.value,apMaterno.value, curp.value, fechaInicio.value, fechaTermino.value);
    return objData;

}
//Page search
async function busquedaPerfilesPH(){

    console.log(JSON.stringify(formSeach()))
    document.getElementById('btnDescargar').hidden = true
    document.getElementById('text-dwnl-text').hidden = false
    ...
    let id = await $.ajax({
        type: 'POST',
        url:'/reclutamientoProfesorHoras/busquedaPerfiPH',
        data:{
            _token: $('meta[name="csrf-token"]').attr("content"),
    ...
    let id2 = await $.ajax({
        type: 'POST',
        url:'/reclutamientoProfesorHoras/busquedaPerfiAcDomiciliosPROFE',
        data:{
            _token: $('meta[name="csrf-token"]').attr("content"),
    ...
    console.log(id2)
    await devolverTablaPROFESOR(id, id2);
    document.getElementById('btnDescargar').hidden = false
    document.getElementById('text-dwnl-text').hidden = true
}



const  devolverTablaPROFESOR = async (datosEmp, datosDom)=>{
    ...
    let data =  `<table class="table table-striped table-responsive" id="tableResultadoEXCEL">
                        <thead>
                            <tr>
                                ...
                                ${obtainsTitlePROFESOR(datosEmp)}
                            </tr>
                        </thead>

                        <tbody>
                            ${unionDataPROFESOR(datosEmp, datosDom)}
                        </tbody>
                    </table>`;
...
const obtainsTitlePROFESOR = (datosEmp) =>{
...
const unionDataPROFESOR = (datosEmp, datosDom) =>{
    ...
    data.forEach((item)=>{
        dataReturn += `<tr>
                                ...
                                ${ObtenerFechaActivacionPROFESOR(item, result1)}
                                ${ObtenerInformacionPersonaPROFESOR(item, infoPersona)}
                                ${ObtenerTipoSangrePROFESOR(item, tipoSangre)}
                                ${ObtenerSegSocialPROFESOR(item, segSocial)}
                                ${ObtenerDomicilioPROFESOR(item, datosDom)}
                                ${ObtenerDatosNacimientoPROFESOR(item, datosNacimiento)}
                                ${ObtenerCorreoPROFESOR(item, result1)}
                                <td>${ObtenerTelefonoPROFESOR(item, numTelefonos)}</td>
                                ${ObtenerEscolaridadMaxPROFESOR(item, maxEscolaridad)}
                                ${ObtenerFamiliaresPROFESOR(item, infoParientes)}
                            </tr>`;
    });
    return dataReturn;
}

const ObtenerFechaActivacionPROFESOR =(idPersona, result1)=>{
...
const ObtenerDomicilioPROFESOR = (idPersona, datosDom)=>{
...
const ObtenerFamiliaresPROFESOR = (idPersona, infoParientes) =>{
...
const ObtenerEscolaridadMaxPROFESOR = (idPersona, maxEscolaridad)=> {
...
const ObtenerTipoSangrePROFESOR = (idPersona, tipoSangre) =>{
...
const ObtenerSegSocialPROFESOR = (idPersona, segSocial) =>{
...
const ObtenerCorreoPROFESOR = (idPersona, datosCorreo) =>{
...
const ObtenerDatosNacimientoPROFESOR = (idPersona, datosNacimiento) =>{
...
const ObtenerTelefonoPROFESOR = (idPersona, numTelefonos) =>{
...
const ObtenerInformacionPersonaPROFESOR = (idPersona, infoPersona) =>{
