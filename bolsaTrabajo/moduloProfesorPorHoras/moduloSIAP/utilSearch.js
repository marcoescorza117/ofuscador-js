...
const getDataServer = async (url, type, formData = null) => {
    let id = await $.ajax({
        type: type,
        url: '/reclutamientoProfesorHoras/' + url,
        data: {
    ...
    if (loosUnicos.length>0){
        ...
        if (segunda.pais  === " " && tercero.nivelEstudio === " " && cuarto.distincionCientifica === " " && quinto.idioma === " "){
            ...
            dataaa.forEach((item)=>{
                ...
                stringGenerator+= `
                <tr>
                    <td class="sorting_1"><div class="btn-group mr-2"><a title="Ver detalles" class="btn btn-secondary boton" href="/reclutamientoProfesorHoras/obtenerPersonaAvanzado/${item.idPersona}"><i class="fas fa-id-card"></i>   </a></div></td>
...
const validatorFinal =(usuario)=>{
    ...
    if (usuario===undefined){
        ...
        if (comprobadorGet){
            ...
            return `<tr>
                    <td class="sorting_1"><div class="btn-group mr-2"><a title="Ver detalles" class="btn btn-secondary boton" href="/reclutamientoProfesorHoras/obtenerPersonaAvanzado/${usuario.idPersona}"><i class="fas fa-id-card"></i>   </a></div></td>
