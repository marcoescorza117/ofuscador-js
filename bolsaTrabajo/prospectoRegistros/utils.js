...
const checkValues = (value) => {
    ...
    if (value === null || value.trim() === '') {
        return "Por favor, agregue los datos correspondientes";
    }
    return value;
}
...
const checkInitialvalues = (value) => {
    if (value === null || value === undefined) {
        return "Por favor, agregue el dato correspondiente.";
    }
...
const sendFilePDF = async (itemSelectDoc, idDocType, idCurriculumFormacion) => {
    ...
    let id = await $.ajax({
        ...
        type: "POST",
        url: '/prospecto/setDocumentacion',
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
...
const agregarDatePickerFormacionAdicional = (elementID) =>{
    //Copiamdo el codigo porque me dam amsiedad prograrlo tomdo :V
    let anio = new Date().getFullYear();

    let anioNuevo = anio-5


    $('#' + elementID).datepicker({
        uiLibrary: 'bootstrap4',
        iconsLibrary: 'fontawesome',
        minDate: new Date( new Date().setFullYear(anioNuevo)),
        dateFormat: "mm-dd-yy"
...
async function retornaNombreMunicipio(value) {
    ...
    let idName = await getDataFromServerHidden('obtenerMunicipio', 'POST', data)
    //console.log(idName)
    if (idName.errors) {
...
async function getDatosComunicacion() {
    let info = await getDataFromServerHidden('obtenerDatosComunicacion', 'GET');
    //console.log(info)
    return info;
...
async function getDatosComunicacionCorreo() {
    let info = await getDataFromServerHidden('obtenerDatosComunicacionCorreo', 'GET');
    //console.log(info)
    return info;
}
...
async function getCurrentAddress() {
    let info = await getDataFromServerHidden('obtenerDatosDomicilio', 'get');
    return info;
}

//1.- Primer caso, se agregan datos de domicilio principal, nacional
//FuncionAgregar
async function agregarDireccionActual() {

    let array = [];

    let object = {

        tipoDomicilio: 'Privado',
        idEntidadFederativa: '4',
        idMunicipio: '1059',
        ciudad: 'Mineral de la Reformaaaaaaa', // se copia del municipio (del select---text)
        //localidad:'Mineral de la Reforma HIDALGO',
        asentamiento: 'Colonia la Providencias',
        codigoPostal: '42185',
        calle: 'Avenida Cocos',
        numeroExterior: '10166',
        numeroInterior: '10166'
    ...
    let info = await getDataFromServerHidden('guardarDomicilioResidenciaN', 'POST', data);
    console.log(info);
}
async function editarDireccionActualN() {
    let array = [];

    let object = {

        idPersonaDomicilio: 'eyJpdiI6Ilc4WDQxbDMwcFQxOUFLRHlTWHFFSUE9PSIsInZhbHVlIjoiTzBOWTNEOTdTUWlRckhFVWtqN0hQZz09IiwibWFjIjoiZDExYWVhOWYwZWRhNjBkZDdjMDQ5NjAzM2Q4NzlkODVkMThlYWExYTk0ODQ3OWZlZjhjZGQwZTBmNWM3MTFhOCJ9',
        tipoDomicilio: 'Privado',
        idEntidadFederativa: '4',
        idMunicipio: '1059',
        ciudad: 'Mineral de la ReformaHGOMEX', // se copia del municipio (del select---text)
        //localidad:'Mineral de la Reforma HIDALGO',
        asentamiento: 'Colonia la ProvidenciasHidalgo',
        codigoPostal: '42185',
        calle: 'Avenida Cocos Palmeras',
        numeroExterior: '10166',
        numeroInterior: '10166'
    ...
    let info = await getDataFromServerHidden('editarDomicilioResidenciaN', 'POST', data);
    console.log(info);
}


//Generar objeto para resolver get

//Devuelve objeto para agregarlo a repository de domicilio....
const devolverHorario = () => {
    return [
        {
            asentamiento: "Colonia la ProvidenciasHidalgo",
            calle: "Avenida Cocos Palmeras",
            codigoPostal: "42185",
            domicilio: "Mineral de la ReformaHGOMEX|Mineral de la ReformaHGOMEX|Colonia la ProvidenciasHidalgo|42185|Avenida Cocos Palmeras|10166|10166",
            entidadFederativa: "Hidalgo",
            idAsentamiento: "0",
            idDomicilio: "eyJpdiI6IlBPM1RrTThoTWh1c08rQzhMVnNNNHc9PSIsInZhbHVlIjoieW5aMXRkeWIzcXA1MHVzWnRlSkErUT09IiwibWFjIjoiMjg5Yjc5MmRlZDM3NjI1NjM0ZmYwZDc0MTUzY2ZmZDk3MzFhOTdmYTIzNGVkNGE5Y2RhNjU4NmM2ODNhNDZhMCJ9",
            idDomicilioComunicacion: "eyJpdiI6IkdVOFN5SndrQUpcL1ZTRjBZUUttMFd3PT0iLCJ2YWx1ZSI6IndIUGo1N09cL3BjQ1lmcGZKRWx3XC90UT09IiwibWFjIjoiZDgwNWFlYjkzYWZjOTg4ZDUyY2E0M2EzZTA2MWViZTY3ZjNhOTdjYjMyMzRiYjRmOWRmNmM5OTA4MTk5YzhhYSJ9",
            idEntidadFederativa: "4",
            idMunicipio: "1059",
            idPais: "1",
            idPersona: "1020",
            idPersonaDomicilio: "eyJpdiI6InBTWEIrYmQxSzA5QzZHWUcySGU5bnc9PSIsInZhbHVlIjoiTmlWZ01wRHdVS2I4UzRUcTBzK0ZPdz09IiwibWFjIjoiNDVjYmE5YTYyOWMxNzM3MGFiODg5ODFmNDFlMTBjNDVhMDgzYzVlZjNmZjNkNTA2YjZiN2ViOWE2NGE0MDJlNSJ9",
            idPrincipal: "S",
            idTipoDomicilio: "13",
            idValidado: "P",
        ...
        {
            asentamiento: "Colonia la Providencias",
            calle: "Avenida Palmeras",
            codigoPostal: "42185",
            domicilio: "Mineral de la Reforma HIDALGO|Mineral de la Reforma HIDALGO|Colonia la Providencias|42185|Avenida Palmeras|1009|1009",
            entidadFederativa: "Hidalgo",
            idAsentamiento: "0",
            idDomicilio: "eyJpdiI6ImZ0U2JDYzRaejhONXY4ZEJhMnJTQ3c9PSIsInZhbHVlIjoiYW5rNFBzV3VNWm9sT3NXSnJmTTlxZz09IiwibWFjIjoiM2QwMWFmNjg0NTMxN2MwMDM3YTYyMzQ5YTdhODJjYjZlYTAwYmY4OTZiMTEzNGNlNzMzZjI3MjgzYmYxN2JhOCJ9",
            idDomicilioComunicacion: "eyJpdiI6Ikl1NUp0c2VXUEFvd1Q1Y1R0eE1VS2c9PSIsInZhbHVlIjoiZXo1dTVsWG9hb1lJTmtDZHl3Yk1JZz09IiwibWFjIjoiNTMwZjgxY2Y1MTA3MjYwMGU0NzI1YWQ4MDQ0MGQwNmMxYjE4MDFjMGM5N2Q0YWYyNzhkMGRlMGMyZWNmNDA0YSJ9",
            idEntidadFederativa: "4",
            idMunicipio: "1059",
            idPais: "1",
            idPersona: "1020",
            idPersonaDomicilio: "eyJpdiI6IkU1VmpaeHdKTXZnbmlQQzMxMTlqbUE9PSIsInZhbHVlIjoiVlVwaDV2XC92a3JHVXVodG1ZTjBYaFE9PSIsIm1hYyI6ImUyYWUzODkxYWRjOTE4YTc0NmIwNWJmNTE3NGJmYmI3YzlhOGFkNzJiY2NkMDU3ODAyYjliOWU4N2Y5OTczNDMifQ==",
            idPrincipal: "N",
            idTipoDomicilio: "13",
            idValidado: "P",
            municipio: "Mineral de La Reforma",
                        ...
                        <div class="card-header-titles  card-header-sectionTitle" >
                            <div class="card-header-title">
                                <h6 style="font-weight: 300" onmouseover="">
                                    Domicilio alterno  <span><i class="fas fa-home"></i></span>
                                </h6>
                            </div>
                            <div class="card-header-buttons">
                                ${btnsEditarDomAlternoNuevo()}
                            </div>
                        </div>
                    </div>
...
const blockEventLoop = (itemGeneral, state) =>{

    //Find all the button elements and disable them
