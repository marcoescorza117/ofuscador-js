//
function componenteInicialBaseDocumentacionPersonal(itemNacion) {
            ...
            <div class="card-body card-body-back" id="card-componentBody" >
                ...
                <div class="container-form">
                    ...
                    <form action="" name="formacion-DocumentacionPersonal-form${j}" id="formacion-DocumentacionPersonal-form${j}">
                            ...
                            <!--Formulario/Info-->
                        ${formularioNuevoDocumentacionPersonal(itemNacion)}
...
function formularioNuevoDocumentacionPersonal(itemNacion) {
        ...
        <div class="form-row">
            ...
            <h6>Tipo de documento:</h6>
                <select class="browser-default custom-select" name="tipoDocumento" id="tipoDocumento${j}">
                    ${distinguirTipoNacionalidad(itemNacion)}
                </select>
...
function setSelectedDocument(value){
    ...
    var obje = [
        ...
        {valueSelect:"27",description:"Certificado médico"}
    ]

    //Hacer identificacion de Documentos para nacionales y extranjeros...

    let identificadorNacion = document.getElementById('ValorNacionalidad').value

    //Nacional
    if (identificadorNacion === "1"){
        obje = [];
        obje = [
            {valueSelect:"2", description:"Acta de nacimiento"},
            {valueSelect:"6", description:"Comprobante de afiliación al IMSS"},
    ...
    if (value === null || value === '') {
        optionsItems += `<option value="" selected disabled>Seleccione una opción</option>`;
        obje.forEach((item) => {
        ...
        //optionsItems += `<option value="">Seleccione una opción</option>`;
        obje.forEach((item) => {
...
const obtenerEnlaceDocumentoDOCPERSONAL = (item) =>{
    ...
    try {
        ...
        return `<a target="_blank" href="/prospecto/documentoPersonalObtener/${formacionActual.idDocumento}">${formacionActual.archivo}</a>`;
    }catch (e){
        return `<p>SIN DOCUMENTO</p>`
    }
}
...
const agregarNuevoElementoDocumentacionPersonal = (itemNacion) => {

    //Buscando elemento origen....

    $("#formacionDocumentacionContainer").prepend(componenteInicialBaseDocumentacionPersonal(itemNacion))
...
function distinguirTipoNacionalidad(itemNacion){
    let ValorNacion = document.getElementById('ValorNacionalidad').value
    console.log(ValorNacion)

    if (String(ValorNacion) === "1"){
        console.error("Nacional")
        return retonarOpcionesNacionales();
    }else{
        console.error("Extranjero")
        return retornarOpcionesExtranjeros();
    }
}


function retonarOpcionesNacionales (){

    return `<option selected="" disabled="">Seleccione</option>
            ...
            <option value="27">Certificado médico</option>`;
}


function retornarOpcionesExtranjeros() {
        ...
        <div class="card  card-formacion-academica card-formacion-DocumentacionPersonal" id="card-formacion-DocumentacionPersonal">
            <div class="card-header">
                ...
                <div class="card-buttons">
                        ...
                        ${btnsComponentEditaFirstDocumentacionPersonalMOSTRAR(itemFormacion)}
...
async function mostrarFormulariosDocumentacionPersonal(){
    ...
    let data = await getDataFromServerHidden('getDocumentacionPersonaGeneral','GET');
    //Obteniendo datos de nacionalidad

    let data2 = await getDataFromServerHidden('getDistincionPersonaNacion','GET');
    document.getElementById('formacionDocumentacionContainer').innerHTML = " "
    console.log(data2.length);
    if (data2.length === 0){

        $("#botonDistincionNacion").append("")
        $("#formacionDocumentacionContainer").append(`
        <p class="text-danger text-center">
            *Para poder llenar la información de esta sección,
            primero tendrá que rellenar la información de la sección
            "Datos de Nacimiento"
        </p>
        <br>

        <small class="text-center text-muted">
            ***Dé click <a href='javascript:;' onclick="showDocumentacionPersonal()">aquí</a>
            una vez que haya completado la información requerida.
        </small>
`);
    }else{
        let valorNacion = document.getElementById('ValorNacionalidad')
        let informacionNacimiento = data2[0];
        if (String(informacionNacimiento.idPais) === "1"){
            console.log("Es mex")
            valorNacion.value = "1"
            PrivateNewWriter( data, "1" )

        }else{
            console.log("Es extranjero")
            valorNacion.value = "0"
            PrivateNewWriter( data, "0" )
        }
    }
}


const PrivateNewWriter =( data , idDistincion) =>{
    ...
    botonDistincion(idDistincion)
    $("#formacionDocumentacionContainer").prepend(dataAdicionalHTML);
}


const botonDistincion = (itemNacion) => {

    let data = `
    <div id="container-title-btn" class="container-mainly-btn">
        <button type="button" id="gral-options-agregar" class="btn btn-outline-secondary btn-sm" onclick="agregarNuevoElementoDocumentacionPersonal(${itemNacion})"><i class="fas fa-plus"></i> Agregar</button>
    </div>
    `
    $("#botonDistincionNacion").append(data)
